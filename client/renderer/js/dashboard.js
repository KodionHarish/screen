...
//     // Request current admin toggle status
//     socket.emit("request-admin-toggle-status", { userId: currentUserId });
//   });
...
//   // // ✅ Handle admin toggle updates
//   // socket.on("toggle-updated", ({ userId: targetUserId, toggled }) => {
//   //   console.log("🚦 Real-time toggle changed:", toggled);

//   //   // Update the admin tracking state
//   //   adminTrackingEnabled = toggled;

//   //   // Store in localStorage for persistence
//   //   localStorage.setItem(`adminTracking_${currentUserId}`, toggled.toString());
...
//   // ✅ Handle initial admin toggle status response
...
//     }
//   }, 1000000); 
//   // Also emit when tracking status changes
...
//   // Listen for admin toggle updates
...
// let socket;
// let currentUserId = null;
// console.log(socket,"sjhdsjahdjasdajs")
...
//         notificationIcon.addEventListener("click", (e) => {
//             e.stopPropagation();
//             window.electronAPI.toggleNotificationPanel();
//         });
//     }
...
//     // Tracking toggle - need to use the exposed function from tracking.js
//     if (trackingToggleBtn) {
//         trackingToggleBtn.addEventListener("click", async () => {
//             // Get the latest adminTrackingEnabled state
//             const adminTrackingEnabled = window.electronAPI.getAdminTrackingEnabled();
//             const { isTrackingActive } = window.electronAPI.getTrackingState();
...
//                 if (result.success) {
//                     window.electronAPI.setTrackingState(result.isActive);
//                     window.electronAPI.updateTrackingDisplay({ totalTime: result.totalTime, currentSessionTime: result.currentSessionTime || 0 });

//                     if (socket && currentUserId) {
//                         socket.emit("user-tracking-status", {
//                             userId: currentUserId,
//                             isTracking: result.isActive
//                         });
//                     }
//                 } else {
...
//     window.electronAPI.onTrackingStatusChanged((data) => {
//         console.log("📊 Tracking status changed:", data);
//         window.electronAPI.setTrackingState(data.isActive);
//         window.electronAPI.updateTrackingDisplay(data);

//         if (socket && currentUserId) {
//             socket.emit("user-tracking-status", {
//                 userId: currentUserId,
//                 isTracking: data.isActive
//             });
//         }
//     });
...
//     window.electronAPI.onToggleUpdated((data) => {
//         console.log("Admin toggle update received:", data);
//         window.electronAPI.setAdminTrackingEnabledAdmin(data.toggled);
//         window.electronAPI.setAdminTrackingEnabledTracker(data.toggled);
//         window.electronAPI.updateAdminToggleButton();
//     });

//     window.electronAPI.onKeyPressUpdate((data) => {
//         // Update UI for key presses
//         updateDetailedStats(data);
//     });

//     window.electronAPI.onMouseClickUpdate((data) => {
//         // Update UI for mouse clicks
//         updateDetailedStats(data);
//     });

//     window.electronAPI.onDetailedStatsUpdate((data) => {
//         // Update UI components for detailed stats
//         updateDetailedStats(data);
//     });
// }

// function updateDetailedStats(data) {
//     // Update detailed stats UI
//     const totalKeyPresses = document.getElementById("totalKeyPresses");
//     const totalMouseClicks = document.getElementById("totalMouseClicks");
//     const topKeysList = document.getElementById("topKeysList");

//     if (totalKeyPresses && data.totalKeyPresses !== undefined) {
//         totalKeyPresses.textContent = data.totalKeyPresses;
//     }

//     if (totalMouseClicks && data.totalMouseClicks !== undefined) {
//         totalMouseClicks.textContent = data.totalMouseClicks;
//     }

//     if (topKeysList && data.keyPressDetails) {
//         const topKeys = window.electronAPI.getTopKeys(data.keyPressDetails, 5);
//         topKeysList.innerHTML = topKeys.map(({key, count}) =>
//             `<li>${key}: ${count} times</li>`
//         ).join('');
//     }
// }
...
//     try {
//         // Create styles first
//         window.electronAPI.createNotificationStyles();
...
//         // Initialize admin tracking state
//         const storedAdminTracking = localStorage.getItem(`adminTracking_${currentUserId}`);
//         const isAdminEnabled = storedAdminTracking === 'true';
//         window.electronAPI.setAdminTrackingEnabledAdmin(isAdminEnabled);
//         window.electronAPI.setAdminTrackingEnabledTracker(isAdminEnabled);

//         // Initialize socket
//         socket = window.electronAPI.initializeSocket(currentUserId);

//         // Get tracking status
//         const trackingStatus = await window.electronAPI.getTrackingStatus();
//         window.electronAPI.setTrackingState(trackingStatus.isActive);

//         renderDashboardUI(data, trackingStatus);

//         // Initialize elements and event listeners
//         window.electronAPI.initializeTrackingElements();
//         window.electronAPI.updateAdminToggleButton();
//         setupEventListeners();
...
//         const existingMemo = await window.electronAPI.getMemo();
//         if (existingMemo) {
//             document.getElementById("memoInput").value = existingMemo;
//         }

//         // Welcome notification
//         setTimeout(() => {
//             window.electronAPI.addNotification("Welcome!", `Hello ${data.name}! Your dashboard is ready.`, "info");
//         }, 1000);

//         // Request initial detailed stats
//         const detailedStats = await window.electronAPI.requestDetailedStats();
//         console.log("Initial detailed stats:", detailedStats);
//         updateDetailedStats(detailedStats);

//     } catch (err) {
//         console.error("❌ Error loading dashboard:", err);
//         window.electronAPI.addNotification("Error", "Failed to load dashboard. Redirecting to login...", "error");
//         setTimeout(() => window.location.href = "login.html", 2000);
//     }
// }

// function renderDashboardUI(data, trackingStatus) {
//     const userInfo = document.getElementById("userInfo");
//     userInfo.innerHTML = `
//         <header class="header-sec">
//             <div class="header">
//                 <h1>Hello, ${data.name}!</h1>
//                 <p>Welcome to your tracking dashboard.</p>
//             </div>
//             <div class="logout-section">
//                 <div class="notification-icon-container" id="notificationIcon">
//                     <span class="notification-icon">🔔</span>
...
//         <div class="admin-banner" id="adminToggleBanner" style="display: none;"></div>

//         <main>
//             <section class="tracking-section card">
//                 <h2>Activity Tracking</h2>
//                 <div class="tracking-summary">
//                     <div id="trackingStatusDisplay" class="status-display"></div>
//                     <div id="trackingTimeDisplay" class="time-display"></div>
//                 </div>
//                 <button id="trackingToggleBtn" class="tracking-button">Loading...</button>
//             </section>

//             <section class="memo-section card">
//                 <h2>Memo</h2>
//                 <textarea id="memoInput" placeholder="Write your notes here..."></textarea>
//             </section>

//             <section class="detailed-stats-section card">
//                 <h2>Detailed Activity Stats</h2>
//                 <div class="stats-grid">
//                     <div class="stat-item">
//                         <h3>Total Key Presses:</h3>
//                         <p id="totalKeyPresses">0</p>
//                     </div>
//                     <div class="stat-item">
//                         <h3>Total Mouse Clicks:</h3>
//                         <p id="totalMouseClicks">0</p>
//                     </div>
//                     <div class="stat-item large">
//                         <h3>Top Pressed Keys:</h3>
//                         <ul id="topKeysList"></ul>
//                     </div>
//                 </div>
//             </section>

//             <section class="admin-toggle-section card">
//                 <div class="admin-toggle-header">
//                     <span class="admin-toggle-icon">⚙️</span>
//                     <h3 class="admin-toggle-title">Admin Tracking Control</h3>
//                 </div>
//                 <p class="admin-toggle-description">Manage user activity tracking status from here. This setting overrides individual user controls.</p>
//                 <div class="admin-toggle-control">
//                     <div>
//                         <div id="adminToggleStatus" class="toggle-status disabled">Tracking Disabled</div>
//                         <div class="toggle-info">Admin controlled tracking status.</div>
//                     </div>
//                     <button id="adminToggleBtn" class="admin-toggle-btn disabled"></button>
//                 </div>
//             </section>
//         </main>
...
//                 <span class="notification-panel-title">Notifications</span>
//                 <div class="notification-panel-actions">
//                     <button class="clear-all-btn" id="clearAllNotificationsBtn">Clear All</button>
//                 </div>
//             </div>
...
// // Wait for the Electron API to be ready before initializing
// function waitForElectronAPI() {
//     if (window.electronAPI) {
//         initializeDashboard();
//     } else {
//         // Listen for the API ready message
//         window.addEventListener('message', (event) => {
//             if (event.data.type === 'ELECTRON_API_READY') {
//                 initializeDashboard();
//             }
//         });
...
//                 initializeDashboard();
//             } else {
//                 console.error("Electron API not ready after timeout");
//             }
//         }, 2000);
//     }
// }
